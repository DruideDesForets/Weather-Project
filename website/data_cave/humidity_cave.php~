...
$req_maison=$bdd->query('SELECT humidity, YEAR(date) AS year, MONTH(date) AS month, DAY(date) AS day, HOUR(date) AS hour, MINUTE(date) AS minute FROM maison');
...
echo '];';
    ...
    ?>
    Highcharts.chart('container_humidity_maison', {
        chart: {
            zoomType: 'x'
        },
        title: {
